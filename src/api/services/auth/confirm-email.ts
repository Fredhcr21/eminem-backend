...
export const confirmEmail = async (token: string, req: Request): Promise<ResponseSuccess> => {
  ...
  // If not such user exists, their token is expired, bail.
  ...
  if (user.emailStatus === 'unconfirmed') {
    ...
    if (!user.emailChangeCandidate) {
      throw new Error(
        `Consistency violation: Coudl not update Stripe customer because this user record's emailChangeCandidate ("${user.emailChangeCandidate}") is missing. (This should never happen.)`,
      );
    }

    /**
     * Last line of defense: since email change condidates are not protected
     ...
     * last checked it's availability. (This is a relatively rare edge case--
